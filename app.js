'use strict';

var app = angular.module('myApp', []);


app.controller('mainCtrl', function($scope, $http) {
    // console.log('mainCtrl loaded');

    $scope.getPhoto = () => {
        $http({
                url: "https://ie-public-safety.run.aws-usw02-pr.ice.predix.io/v1/assets/1000000018/media?media-types=IMAGE&start-ts=1453832741281&end-ts=1462996283887&size=10&page=1",
                method: "GET",
                headers: {
                    'Authorization': 'Bearer eyJhbGciOiJSUzI1NiJ9..VlUqRBMqYwEPc8wEKcp4L0gy3m8aY-v0x0nCHD0oONjkDvcqXWQiANq0KL1x-LFMJhvXApmYtKvrlJSKZSEhRHUWqxJDfJVVYaibjwLScuBUctCvh6v8_9kBobQ5AIAszaJkSe210EPdNbTp0k5BGbN4UwvRUU-280rWZ6eJyXpQKVwz6EETNly7nmXmCk9eN8ImYaQ_gMqPJ-a2IlEBBAkq0TJcFYcdb9gHoKSPV2-rz8EOgUbVO3tO7s-so1gBkiej0Nxl0_-JE43izF2OMThRCCBCFvGlUpsrd3xhvWUWoWgvNtydNEUYYKMzmNF-52IHOQedzBj-9qBZ_uotDw',
                    'Predix-Zone-Id': 'dc4829a9-3087-414a-9ab2-4dbf6f264263'
                }
            })
            .then(res => {
                console.log('res: ', res);
                console.log('res11: ', res.data._embedded.medias);
                $scope.images = res.data._embedded.medias;
            })
            .catch(err => {
                console.log('err: ', err);
            })
    }

    $scope.getPhoto = () => {
         $http({
                url: "http://ie-media-service.run.aws-usw02-pr.ice.predix.io/media/file/1000000018_1461873807278_IMAGE",
                method: "GET",
                headers: {
                    'Authorization': 'Bearer eyJhbGciOiJSUzI1NiJ9..VlUqRBMqYwEPc8wEKcp4L0gy3m8aY-v0x0nCHD0oONjkDvcqXWQiANq0KL1x-LFMJhvXApmYtKvrlJSKZSEhRHUWqxJDfJVVYaibjwLScuBUctCvh6v8_9kBobQ5AIAszaJkSe210EPdNbTp0k5BGbN4UwvRUU-280rWZ6eJyXpQKVwz6EETNly7nmXmCk9eN8ImYaQ_gMqPJ-a2IlEBBAkq0TJcFYcdb9gHoKSPV2-rz8EOgUbVO3tO7s-so1gBkiej0Nxl0_-JE43izF2OMThRCCBCFvGlUpsrd3xhvWUWoWgvNtydNEUYYKMzmNF-52IHOQedzBj-9qBZ_uotDw',
                    'Predix-Zone-Id': 'dc4829a9-3087-414a-9ab2-4dbf6f264263'
                }
            })
            .then(res => {
                 console.log('res: ', res);
                $scope.image = res.data
            })
            .catch(err => {
                 console.log('err: ', err);
            })

    }
});
